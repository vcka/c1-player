...
//		codecMutex.Unlock();
	...
	int r = ioctl(handle, AMSTREAM_IOC_VFORMAT, amlFormat); //vcka
	if (r < 0)
	{
		codecMutex.Unlock();
		throw Exception("AMSTREAM_IOC_SET failed.");
...
//	}
	...
	*/
	...
	r = ioctl(handle, AMSTREAM_IOC_PORT_INIT, (unsigned long)0);
	if (r != 0)
	{
		codecMutex.Unlock();
...
double AmlCodec::GetCurrentPts()
{
	...
	//parm.cmd = AMSTREAM_IOC_VPTS;
	//parm.data_32 = &vpts;
	//printf("CHEKI'n");
	//int ret = ioctl(handle, AMSTREAM_IOC_GET, (unsigned long)&parm);
	int ret = ioctl(handle, AMSTREAM_IOC_VPTS, (long unsigned int)&vpts);
	if (ret < 0)
	{
		printf("AmlCodec::GetCurrentPts() parm.data_32=%u",
		vpts);
		codecMutex.Unlock();
	...
	codecMutex.Unlock();
	//double vpts2=(double)vpts;
	//	printf("AmlCodec::GetCurrentPts() parm.data_32=%u",
	//	vpts / PTS_FREQ);
	return (double)vpts / (double)PTS_FREQ;
}
...
buf_status AmlCodec::GetBufferStatus()
{
	...
	//if (true) 
	{
		//struct buf_status *buf;//new
		buf_status status; //old
		//struct am_io_param am_io;//new
		...
		//am_io_param am_io;
		//parm.cmd = AMSTREAM_IOC_VB_STATUS;
		
		//codecMutex.Lock();

		int r = ioctl(handle, AMSTREAM_IOC_VB_STATUS, (unsigned long)&parm);  //vcka param
		...
		memcpy(&status, &parm.status, sizeof(status)); //old
		//memcpy(buf, &am_io.status, sizeof(*buf)); //new
...
void AmlCodec::CheckinPts(unsigned long pts)
{
	...
	am_ioctl_parm parm = { 0 };
	//parm.cmd = AMSTREAM_IOC_TSTAMP;
	parm.data_32 = (unsigned int)pts;

	int r = ioctl(handle, AMSTREAM_IOC_TSTAMP, pts); //vcka
	if (r < 0)
	{
		codecMutex.Unlock();
